...
def test_eager_end_of_turn_is_off_by_default() -> None:
    ...
    assert Settings().deepgram_stt_eager_eot_threshold is None


def test_eager_end_of_turn_can_be_switched_on(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("PERSONAE_DEEPGRAM_STT_EAGER_EOT_THRESHOLD", "0.4")
...
def test_an_eager_threshold_above_the_final_one_is_refused(
    ...
    monkeypatch.setenv("PERSONAE_DEEPGRAM_STT_EOT_THRESHOLD", "0.7")
    monkeypatch.setenv("PERSONAE_DEEPGRAM_STT_EAGER_EOT_THRESHOLD", "0.8")
...
def test_eager_end_of_turn_can_be_switched_off(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("PERSONAE_DEEPGRAM_STT_EAGER_EOT_THRESHOLD", "")
    assert Settings().deepgram_stt_eager_eot_threshold is None
