...
def test_a_language_nova_3_cannot_do_is_refused(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("PERSONAE_DEEPGRAM_STT_LANGUAGE", "fr")
...
def test_a_language_nova_3_supports_is_accepted(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("PERSONAE_DEEPGRAM_STT_LANGUAGE", "de")
    monkeypatch.setenv("PERSONAE_DEEPGRAM_STT_MODEL", "nova-3")
    monkeypatch.setenv("PERSONAE_DEEPGRAM_TTS_VOICE", "aura-2-thalia-en")
    assert Settings().deepgram_stt_language == "de"
